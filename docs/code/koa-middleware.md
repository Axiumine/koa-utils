...
Koa **router** handler (terminal — no `next`) for the email-verification link, e.g. `GET /check/verify-email/:email/:hash`. Reads `email`/`hash` from `ctx.params`, lowercases the email (no `.trim()`, unlike `checkEmailLen`/`checkPwdLen` elsewhere in the codebase), then runs a sequential private validation chain: `userData4VerifyEmail` → `handleIfEmailAlreadyValid` → `handleBadDB` → `handleIfTooMuchRequestsTimes` → `handleIfHashBad` → `handleIfMoreThan3DaysPassed` → `handleIfAccountDeleted` → `handleIfAccountDisabled` → `enableEmailAccess`, and on success redirects to `/x/registration-done`. Any thrown `Error` is caught: its `.message` is treated as a redirect path — if it matches `ALLOW_ENCODED_URLS_AFTER_X` (`/^\/x\/[a-zA-Z0-9._\-%/]+$/`), the handler redirects there (letting the internal `handleIf*` steps drive the failure landing page, e.g. `/x/error?reason=...`); any other message falls back to the generic `/x/error`.
