...
`mailer` is the one **optional** dependency — `IVerifyEmailMailer`, defaulting to `defaultVerifyEmailMailer` ([`verifyEmailMailer`](./lib-access.md#verifyemailmailer)), i.e. SocketLabs. Optional because this factory is public and pre-existing callers pass four keys; the guard chain's internal factories take theirs as required. `createVerifyEmailFlow` passes its own flow-scoped mailer here, so a flow that replaces the sender replaces it for this mutation too.
