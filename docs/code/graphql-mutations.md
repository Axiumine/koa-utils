...
- On mismatch: if `requestTimes` is `undefined` it throws `throwInternalError()` (500); otherwise it calls `incReqTimes(user._id)`, fires (without awaiting) `mailer.wrongHash(uEmail, requestTimes)`, and returns `false`.
- On match: if `dateLastReq` is `undefined` it throws `throwInternalError()` (500). Otherwise it compares `dateLastReq` against "3 days ago" (`StringLib.isoToTimestamp`): if the request is older than 3 days it fires `mailer.hashReqTooOld(uEmail)` (not awaited) and returns `false`; else if `account.deleted` it returns `false` silently; else if `account.disabled` it fires `mailer.accountDisabled(uEmail)` (not awaited) and returns `false`; otherwise it re-checks that no other account already owns that email as `login.email` (`UserBase.countDocuments`) and, if free, calls `confirmNewEmail(user._id, uEmail)` and returns `true` — or returns `false` if the email was claimed in the meantime.
...
Since 5.3.0 the model, the field paths and the two writers come in through `createEmailChangeHashVerifyMutation({ model, paths, confirmNewEmail, incReqTimes, mailer? })`; `emailChangeHashVerify` is that factory applied to `UserBase` with the default paths, so the queries and the projection above are unchanged. The projection is now built from the same map the reads use, which is what structurally prevents a repeat of the missing-`requestTimes` bug. A consumer on another account model builds its own via [`createVerifyEmailFlow`](./lib-access.md).

`mailer` is the one **optional** dependency — `IVerifyEmailMailer`, defaulting to `defaultVerifyEmailMailer` ([`verifyEmailMailer`](./lib-access.md#verifyemailmailer)), which is SocketLabs behind a 15-minute per-address, per-template debounce. Optional because this factory is public and pre-existing callers pass four keys; the guard chain's internal factories take theirs as required. The three notifications above are the same three the unauthenticated verify-email route can reach, so they share that debounce: two wrong hashes for one address inside the window increment the counter twice and mail once. `createVerifyEmailFlow` passes its own flow-scoped mailer here, so a flow's `mailThrottle` covers this mutation too.
