...
Since 5.3.0 the model, the field paths and the two writers come in through `createEmailChangeHashVerifyMutation({ model, paths, confirmNewEmail, incReqTimes })`; `emailChangeHashVerify` is that factory applied to `UserBase` with the default paths, so the queries and the projection above are unchanged. The projection is now built from the same map the reads use, which is what structurally prevents a repeat of the missing-`requestTimes` bug. A consumer on another account model builds its own via [`createVerifyEmailFlow`](./lib-access.md).
