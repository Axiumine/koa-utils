...
Confirms an in-progress email-change request. Lower-cases the email (no `.trim()`, and — unlike the other mutations here — does **not** call `checkEmailLen`/`checkPwdLen`) and looks up `UserBase` by `account.email.newEmailTmp === uEmail`. If no user is found it logs `'email NON trovata '` and returns `false` (marked `@fixme throw` in source — deliberately not "fixed" without coordinating with the owner). If found, it compares `hash` against the stored `account.email.hash`:
