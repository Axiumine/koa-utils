...
**Throws:** `throwErrorWrongUserInput(...)` (400-class GraphQL error) — when `email === ''` ("L'email non può essere vuota") or `email.length > EMAIL_MAX_LEN` ("L'email non può superare i 255 caratteri").
...
**Throws:** `throwErrorWrongUserInput('Wrong enum value')` (400-class GraphQL error) — when `value` is not found among `Object.values(enumObj)`.
