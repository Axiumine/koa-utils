...
- `CLAUDE.md` (root) — repo-wide agent rules: gates, publish policy, doc-sync map, commit style. Nested per-area `CLAUDE.md` files (see §1) carry the local conventions and pitfalls and are read only when that directory is touched. All of them are written in "caveman ultra" style to cut agent context cost; root Rule 0 requires the `caveman:caveman` skill (marketplace `JuliusBrussee/caveman`) to be invoked before any `CLAUDE.md` is edited. The style is confined to `CLAUDE.md` — every other doc stays normal prose.
- `scripts/caveman-claudemd-gate.sh` — machine enforcement for that rule, wired as two `PreToolUse` hooks in `.claude/settings.json`. The `Skill` matcher records that `caveman:caveman` was invoked in the session; the `Write|Edit` matcher denies writes to any file named `CLAUDE.md` when the skill is either missing or not yet invoked, and prints the install command. Fails closed. Escape hatch `SKIP_CAVEMAN_GATE=1`, owner only.
