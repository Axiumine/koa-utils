...
- **Never blanket-deprecate merely-old versions.** Deprecation mean "this version bad", not "not newest". Blanket → consumers ignore warning, and it fire in CI for every lockfile pin — exactly the audience needing the signal for a real defect. `5.7.2`, `5.8.0` stay undeprecated on purpose: superseded, nothing known wrong. (`5.4.0` / `5.4.1` / `5.5.0` were this example until the 5.7.0 mail-amplification fix gave them a real defect → all 3 deprecated now. Example go stale each release — read npm, never this line: `npm view '@axiumine/koa-utils@>=4' deprecated --registry=https://registry.npmjs.org/`.)
