...
- Verify-email guards mail through injected `IVerifyEmailMailer` (`src/lib/access/verifyEmailMailer.mts`), never `new SocketLabsLib()` inline. Bound defaults use `defaultVerifyEmailMailer` = SocketLabs. In spec: build guard from its `create*` with `fakeVerifyEmailMailer()` (`test/helpers/fakeVerifyEmailMailer.mts`), don't stub `SocketLabsLib.prototype`. `mailer` required on all six factories — optional would let new guard fall back to live client by omission.
