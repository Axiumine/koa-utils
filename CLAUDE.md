...
- No `*` range in `peerDependencies`. `*` accept any major → `sharp` 0.34 → 1.0 install silent, break at runtime, no warning. Upper bound only, at major after one repo build against: `"sharp": "<1"`, `"mongoose": "<10"`, `"redis": "<6"`. `keygrip` keep own `^1.1.0`.
