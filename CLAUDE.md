...
- Verify-email guards mail through injected `IVerifyEmailMailer` (`src/lib/access/verifyEmailMailer.mts`), never `new SocketLabsLib()` inline. Bound defaults use `defaultVerifyEmailMailer` = SocketLabs behind **process-wide** 15 min per-address-per-template debounce, and it is on by default. In spec: build guard from its `create*` with `fakeVerifyEmailMailer()` (`test/helpers/fakeVerifyEmailMailer.mts`). Stubbing `SocketLabsLib.prototype` and driving same address twice — or two spec files reaching same template with same address — get second send suppressed and read as bare `expected false to equal true`, order-dependent. Test that must exercise bound default get its own unique address.
- `handleBadDB` throw `EMAIL_CHECK_LINK` like every sibling guard, not `/x/error` it carried through 5.6.1. Two redirect targets on unauthenticated route = account-existence oracle (corrupt record on real account vs unknown address). Distinction live in `Sentry.captureMessage('[handleBadDB] DB ERROR')` only. Do not reintroduce per-guard redirect path.
- `createVerifyEmailFlow`'s `onAbandon` (`'delete'` default / `'soft-delete'` / `'keep'`) and `deleteUserByEmail` set the disposal writer the two guards run. Factory build exactly one writer, use it in guards **and** return it — so `flow.deleteUserByEmail` report policy, reassigning it after the fact do nothing. `deletedValue` default `true` but written verbatim, function called once per write (`Date` column). Guards still throw whatever the policy; never branch on policy inside a guard.
